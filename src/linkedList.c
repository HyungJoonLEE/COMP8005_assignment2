...
int addTLElement(LinkedList* pList, int position, ThreadNode element) {
    int ret = FALSE;
    int i = 0;
    ThreadNode *pPreNode = NULL;
    ThreadNode *pNewNode = NULL;

    if (pList != NULL) {
        if (position >= 0 && position <= pList->currentThreadCount) {
            pNewNode = (ThreadNode*)malloc(sizeof(ThreadNode));
            if (pNewNode != NULL) {
                *pNewNode = element;
                pNewNode->pLink = NULL;

                pPreNode = &(pList->threadNode);
                for (i = 0; i < position; i++) {
                    pPreNode = pPreNode->pLink;
                }
...
ThreadNode* getTLElement(LinkedList* pList, int position) {
    ThreadNode *pReturn = NULL;
    ThreadNode *pNode = NULL;

    if (pList != NULL) {
        if (position >= 0 && position < pList->currentThreadCount) {
            pNode = &(pList->threadNode);
            for (int i = 0; i <= position; i++) {
                pNode = pNode->pLink;
            }
            pReturn = pNode;
